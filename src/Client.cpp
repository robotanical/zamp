#include "Client.hh"
    ...
    void Client::startCientSender(std::shared_ptr<Scene> scene)
    {
        _scene = scene;
    }
    void Client::communicationThread()
    {
       ...
       if (!_scene->IsChanged())  { usleep(10000); continue; }
       _scene->LockAccess();
    ...
    int Client::getContinueLooping()
    {
        return _continueLooping;
    }
    void Client::cancelContinueLooping()
    {
