...
void XMLInterp4Config::ProcessCubeAttrs(const xercesc::Attributes& rAttrs) {
  ...
  Vector3D Scale, Shift, Rot, Trans, RGB;
  istringstream IStrm;
  IStrm.str(sValue_Shift);
  IStrm >> Shift[0] >> Shift[1] >> Shift[2];
  istringstream ScaleStrm;
  ScaleStrm.str(sValue_Scale);
  ScaleStrm >> Scale[0] >> Scale[1] >> Scale[2];
  istringstream RotStrm;
  RotStrm.str(sValue_Rot);
  RotStrm >> Rot[0] >> Rot[1] >> Rot[2];
  istringstream TransStrm;
  TransStrm.str(sValue_Trans);
  TransStrm >> Trans[0] >> Trans[1] >> Trans[2];
  istringstream RGBStrm;
  RGBStrm.str(sValue_RGB);
  RGBStrm >> RGB[0] >> RGB[1] >> RGB[2];
  std::string name = sValue_Name;
  _config->addMobObj(name, Shift, Scale, Rot, Trans, RGB);
  ...
  cout << "ProcessCubeAttrs zakonczone sukcesem";
}
